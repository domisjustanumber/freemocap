...
class RealtimePipelineConnectRequest(BaseModel):
    ...
    realtime_camera_ids: list[CameraIdString] | None = Field(
        ...
        description="Subset of camera IDs to attach to pipeline nodes. If None, all cameras in the group are used. Desktop UI always sends an explicit subset.",
    )
...
class RealtimePipelineCreateResponse(BaseModel):
    ...
    execution_provider: str | None = Field(
        default=None,
        description="Execution provider for skeleton inference (requested at apply time)",
    )
